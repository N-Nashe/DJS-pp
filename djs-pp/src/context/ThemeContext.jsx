...
export const useThemeContext = () => {
  ...
  if (!themeContextValue) {
    throw new Error(
      'useThemeContext must be used within a <ThemeProvider> tag',
    );
  }
...
export const ThemeProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      'userThemePreference',
      JSON.stringify(isUserPreferringDarkMode),
    );
  ...
  const toggleUserThemePreference = () => {
    setIsUserPreferringDarkMode(
      (previousThemePreference) => !previousThemePreference,
    );
  };
